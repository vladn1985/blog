...
class Request extends \yii\web\Request
{
    ...
    {
        $curLangUrl = $this->getUrl(); //полный URL
        $urlList = explode('/', $curLangUrl);
        $langUrl = $urlList[1] ?? null;

        //Удалить метку языка из URL
        if ($langUrl && $langUrl == Yii::$app->language) {
            $curLangUrl = preg_replace("/^\/$langUrl/", '', $curLangUrl);
        }
